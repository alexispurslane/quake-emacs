...
When writing custom configuration in your =user.el=, it is recommended that you separate your configuration out into logical groups of =use-package= declarations (which themselves bundle up individual package installation and configuration neatly), with each group contained within a function. This is precisely what Quake Emacs does — we call these logical units "layers", after the fashion of Doom Emacs and Spacemacs, although they're just regular functions, no boilerplate necessary — and it has a few benefits:
