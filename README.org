...
4. For any languages you want to use, you need two things: tree-sitter support, and a language server. Languages that don't have these things /will/, unfortunately, require the usual external packages you'd use with regular Emacs until they gain support (if they ever do, as some languages such as Common Lisp and Clojure have their own IDE systems that are superior to language servers). For languages that do have these things, there are typically two steps necessary to use Quake Emacs with that language:

   a. Install the language server somewhere in your PATH so Emacs can find it automatically OR download the language server executable to a custom location and then specify that location when you start Eglot, or in a =.dir-locals.el= file.

   b. Open a file in that language and answer "yes" to =treesit-auto= asking whether you want to automatically download, compile, and install the tree sitter grammar.

   However, the second step only works for languages that have a tree-sitter mode wired up to activate when a file with the correct extension is opened, both because that's how =treesit-auto= knows to trigger, and because tree sitter modes are needed to translate the syntax tree created by the tree sitter grammar into font lock and syntax tables for Emacs to understand. Emacs comes with many tree sitter modes built in by default (see the list below) but unfortunately, not all of them are (yet — [[new languages are being added every version!][https://github.com/emacs-mirror/emacs/blob/331610aef0572eacb2846f817e979aa5e29170b7/etc/NEWS#L2142]]). In that case, you'll need to install an external package for that tree sitter mode.
